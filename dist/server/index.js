...
function top(title, back = "/") {
  if (!title) return `<header class="top"><div class="logo"><img class="brand-logo" src="/nanshan-logo.png" alt="南山人壽">NexSpace</div><div style="flex:1"></div><a class="bell" href="/notifications">♧</a></header>`;
...
function home() {
  return `<main class="page">
    ...
    <div class="section-head"><h2>工作台</h2><a href="/features">功能管理 ›</a></div>
    <section class="primary-grid">
      <a class="feature" href="/space"><span class="ico">▣</span><b>會議室預約</b><small>清單、地圖<br>查詢可用時段</small></a>
      <a class="feature" href="/facility"><span class="ico">♧</span><b>公共設施</b><small>點數、紀錄<br>公設預約</small></a>
      <a class="feature" href="/packages"><span class="ico">▤</span><b>郵務包裹</b><small>寄件、收件<br>常用收件人</small></a>
    </section>
...
function more() {
  const extras=[["▦","合約資訊"],["$","繳費記錄"],["▧","資料列印系統"],["P","停車管理系統"],["↗","貨車預約系統"]];
  return `<main class="page"><div class="lead"><h1>更多功能</h1><p>完整工作台與大樓資訊</p></div><div class="section-head"><h2>個人功能</h2><a href="/features">功能管理</a></div><section class="quick-list"><a class="quick" href="/reservations"><span>▤</span>我的預約</a><a class="quick" href="/tickets"><span>⌁</span>我的工單</a><a class="quick" href="/notifications"><span>♧</span>通知中心</a><a class="quick" href="/meal"><span>▦</span>餐券管理</a></section><div class="section-head"><h2>空間地圖</h2></div><section class="quick-list"><a class="quick" href="/facilities"><span>▦</span>大樓設施</a><a class="quick" href="/guide"><span>⌖</span>使用指南</a><a class="quick" href="/contact"><span>♙</span>聯絡我們</a><a class="quick" href="/services"><span>◎</span>服務申請</a></section><div class="section-head"><h2>其他系統</h2></div>${extras.map(x=>`<a class="feature-row" href="/external?name=${encodeURIComponent(x[1])}"><span class="service-ico" style="margin:0">${x[0]}</span><b>${x[1]}</b><span class="pin">›</span></a>`).join("")}</main>`;
}
...
function featureManager(url) {
  ...
  const pinned = swapped ? [["▤","郵務包裹"],["▣","會議室預約"],["♧","公共設施預約"]] : [["▣","會議室預約"],["♧","公共設施預約"],["▤","郵務包裹"]];
  const rest = [["♙","訪客登記","/visitors"],["!","問題反映","/issues"],["☆","活動與社群","/activities"],["◎","服務申請","/services"],["▦","合約資訊","/external?name=合約資訊"],["$","繳費記錄","/external?name=繳費記錄"],["P","停車管理系統","/external?name=停車管理系統"]];
  return `<main class="page"><div class="lead"><h1>功能管理</h1><p>常用功能最多 3 個，可調整順序與釘選項目</p></div><div class="section-head"><h2>常用功能（3/8）</h2><a href="/features?swap=${swapped?0:1}">交換順序</a></div>${pinned.map((x,i)=>`<div class="feature-row"><span class="drag">⠿</span><span class="service-ico" style="margin:0">${x[0]}</span><b>${x[1]}</b><a class="pin" href="/features?swap=${i===0?1:0}">⌖</a></div>`).join("")}<div class="section-head"><h2>功能列表</h2></div>${rest.map(x=>`<a class="feature-row" href="${x[2]}"><span class="drag">⠿</span><span class="service-ico" style="margin:0">${x[0]}</span><b>${x[1]}</b><span class="pin">＋</span></a>`).join("")}<a class="btn primary" href="/?saved=1">儲存</a></main>`;
}
